...
static void gpio_keys_gpio_work_func(struct work_struct *work)
{
	...
	if (bdata->button->wakeup)
		pm_relax(bdata->input->dev.parent);//释放唤醒状态
}
extern bool is_input_dev_resume(void);
static void gpio_keys_gpio_timer(unsigned long _data)
{
	struct gpio_button_data *bdata = (struct gpio_button_data *)_data;
	if(is_input_dev_resume()) //是否唤醒
		schedule_work(&bdata->work);//调度工作队列
	else
...
static irqreturn_t gpio_keys_gpio_isr(int irq, void *dev_id)
{
    ...
    /*如果支持唤醒源,则唤醒系统*/
	if (bdata->button->wakeup)
		pm_stay_awake(bdata->input->dev.parent); //让电源处于唤醒状态
				...
				const struct gpio_keys_button *button)
{
	...
	bdata->input = input;
	bdata->button = button;
	spin_lock_init(&bdata->lock);//自旋锁初始化

	if (gpio_is_valid(button->gpio)) {//gpio是否有效判断
...
gpio_keys_get_devtree_pdata(struct device *dev)
{
	...
	for_each_child_of_node(node, pp) {
		...
		button->gpio = gpio;//gpio引脚号
		button->active_low = flags & OF_GPIO_ACTIVE_LOW;//值为1
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	ddata->pdata = pdata;
	ddata->input = input;
	mutex_init(&ddata->disable_lock);//互斥锁初始化

	platform_set_drvdata(pdev, ddata);
	input_set_drvdata(input, ddata);

	input->name = pdata->name ? : pdev->name;
	input->phys = "gpio-keys/input0";//物理层
    ...
    /*sysfs文件创建*/
    //目录: /sys/devices/soc.0/gpio_keys.102/

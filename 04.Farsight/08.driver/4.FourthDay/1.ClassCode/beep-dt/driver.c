...
static int hello_probe(struct platform_device *pdev)
{
	...
	printk("match ok\n");
	pgpd0con = ioremap(pdev->resource[0].start  ,pdev->resource[0].end  - pdev->resource[0].start );	
	timerbase = ioremap(pdev->resource[1].start ,pdev->resource[1].end  - pdev->resource[1].start  );
